...
test.describe('Candidates tracker filters', () => {
  ...
  test('candidate drawer shows Documents and Timeline tabs', async ({ page }) => {
    ...
    await page.locator('.ent-table tbody tr button').filter({ hasText: /RES-|@|[A-Za-z]/ }).first().click()
    await expect(page.getByRole('button', { name: 'Documents' })).toBeVisible({ timeout: 10_000 })
    await page.getByRole('button', { name: 'Documents' }).click()
    await expect(page.getByText(/document slots|upload/i).first()).toBeVisible({ timeout: 10_000 })
    await page.getByRole('button', { name: 'Timeline' }).click()
    await expect(page.getByText(/activity timeline|no activity/i).first()).toBeVisible({ timeout: 10_000 })
  })
})
