 ...
 * Handles Google OAuth callback — exchanges code for tokens and saves connection.
 * GET /api/oauth/gmail/callback?code=...&state=...
 */
import { NextRequest, NextResponse } from 'next/server'
import { exchangeGmailCode, saveEmailConnection } from '@/lib/email-oauth'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(`${origin}/dashboard?tab=settings&email_connected=gmail`)
  } catch (err) {
