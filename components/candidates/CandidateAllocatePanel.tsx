...
export function CandidateAllocatePanel({
  ...
  const scheduleInterview = (sub?: SubRow, round?: number) => run(async () => {
    const email = (candidateEmail ?? '').trim()
    const when = slot.trim() ? new Date(slot) : null
    if (slot.trim() && (!when || Number.isNaN(when.getTime()))) throw new Error('Pick a valid date and time')
    const job = sub?.job_post_id || jobId || defaultJobId || undefined
    const nextRound = round ?? (interviews.reduce((m, iv) => Math.max(m, Number(iv.round ?? 1)), 0) + 1)
  ...
  const patchInterview = (id: string, body: Record<string, unknown>) => run(async () => {
    ...
    setMsg(status ? `Interview ${labelFor(INTERVIEW_STATUSES, status)}` : 'Interview updated')
  })
    ...
    <div className="px-5 py-4 space-y-4" data-testid="candidate-allocate-panel">
      ...
      {showIvs && interviews.length > 0 && (
        <div>
                ...
                <div className="mt-3 flex flex-wrap items-end gap-2">
                  <label className="text-[10px] font-extrabold uppercase text-slate-600">
                    ...
                    <input
                      ...
                      data-testid="allocate-interview-slot"
                      className="mt-1 block text-xs border border-slate-200 rounded-lg px-2 py-1.5"
                      defaultValue={toLocalInput(iv.scheduled_at)}
                      onBlur={e => {
                        const v = e.target.value
                        if (!v) {
                          if (iv.scheduled_at) void patchInterview(iv.id, { scheduled_at: null, status: 'to_schedule' })
                          return
                        }
                        if (toLocalInput(iv.scheduled_at) === v) return
                        void patchInterview(iv.id, { scheduled_at: new Date(v).toISOString() })
                      }}
                    />
                  </label>
                  <select
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {(['confirmed', 'completed', 'no_show', 'cancelled', 'rejected', 'selected'] as const).map(st => (
                    <button
                      ...
                      type="button"
                      disabled={busy}
                      onClick={() => patchInterview(iv.id, { status: st })}
                      className={`px-2 py-1 rounded-md text-[10px] font-extrabold border ${
          ...
          <div className="mt-3 rounded-xl border border-teal-200 bg-teal-50/40 p-3 flex flex-wrap items-end gap-2">
            <label className="text-[10px] font-extrabold uppercase text-slate-600">
              Add another round
              <input
                type="datetime-local"
                className="mt-1 block text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-white"
                value={slot}
                onChange={e => setSlot(e.target.value)}
              />
            </label>
            <button
              type="button"
              data-testid="allocate-add-round-btn"
              disabled={busy}
              onClick={() => scheduleInterview(shortlistedSubs[0] || latestSub)}
              className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-teal-600 text-white text-[10px] font-extrabold"
            >
              <Calendar className="w-3 h-3" /> Add slot
            </button>
          </div>
        </div>
      )}
              ...
              <li key={o.id} className="rounded-xl border border-slate-200 p-3 space-y-2">
                ...
                {docsOpen && (
                  <DocsUploadPanel
                    ...
                    offerId={o.id}
                    country={o.country_code || 'MY'}
                    employmentType={o.employment_type === 'foreign' ? 'foreign' : 'local'}
