...
export function Job360View({
  ...
  const actionCards = [
    { key: 'posts', label: 'Generate Job Post', desc: 'LinkedIn, WhatsApp, Email, Indeed', color: 'bg-blue-600 hover:bg-blue-500', tab: 'posts' as Job360Tab, tool: null as string | null },
              ...
              {group.tabs.map(t => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => setTab(t)}
                    className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-black tracking-tight transition-all whitespace-nowrap ${
                      tab === t
                        ? 'bg-indigo-600 text-white shadow-sm'
                        : 'text-slate-800 bg-slate-50 border border-slate-200 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-900'
                    }`}
                  >
                ...
                <div className="space-y-4">
                  <div className="rounded-xl border border-indigo-100 bg-indigo-50/40 p-3 space-y-2">
                    ...
                    <p className="text-xs font-bold text-indigo-900/90">
                      Generate LinkedIn, WhatsApp, Email, and Indeed posts from this JD. Posts are saved on the job.
                    </p>
                    <input
                      value={customPrompt}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="relative">
                        <textarea
                          ...
                          value={posts[postTab] ?? ''}
                          rows={14}
                          className="w-full px-3 py-3 rounded-xl border border-slate-200 bg-white text-sm font-medium text-slate-800 leading-relaxed resize-y min-h-[220px] max-h-[480px] overflow-y-auto gen-post-scroll"
                        />
                        <button
                          type="button"
